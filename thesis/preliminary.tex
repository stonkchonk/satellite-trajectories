...
\begin{document}
	...
	\begin{enumerate}
		...
		\item An eigth degree polynomial equation containing the parameters $a$, $b$ and $c$ is defined as follows:
		\begin{equation}
			x^8+ax^6+bx^3+c=0
		\end{equation}
		\noindent
		Of this polynomial, real roots $w$ larger than zero must be determined. (hier weiterschreiben für schritt 8)
		
		\item Determine the unknown parameters $\rho_1$, $\rho_2$ and $\rho_3$ with the following expressions:
		\begin{equation}
			\begin{aligned}
				\rho_1 =
				\frac{1}{D_0}
