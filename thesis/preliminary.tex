...
\begin{document}
	...
	The reference coordinate system of our choice is the Earth Centered Inertial (ECI) coordinate system. This coordinate system is aligned to planet earth but not fixed to its surface. The Z-axis is aligned to earth's rotation axis, while the X-Y-plane is aligned to earth's equatorial plane, as visualized below. Both X-, and Y-axes can be considered fixed in reference to the background stars when dealing with short time periods. For time periods in the range of months and years this is not the case due to precession, as elaborated in section \ref{sec:earth_axis_precession}. Each position can be described by a vector in $\mathbb{R}^3$.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.40\linewidth]{images/Earth_Centered_Inertial_Coordinate_System.png}
	...
	\begin{figure}[H]
		...
		\caption{Stars and a satellite (encircled) are perceived as dots by cameras.}
	...
	\begin{enumerate}
		...
		\item The determination of $T_0$ since the J2000 epoch:\newline
		The J2000 Julian epoch is defined as noon (UT) on January 1st, 2000 and marks the number of days since the start of the Julian calendar, which is 2451545. This step consists of calculating the number of Julian centuries $T_0$ between J2000 and the current observation date. This can be archieved with the following formula containing the result of the previous step:
		\begin{equation}
	...
	\subsection{Factoring in axial precession of planet earth}
	\label{sec:earth_axis_precession}
	Defined previously in section \ref{sec:reference_coordinate}, the ECI coordinate system specifically the Z-axis is aligned with earth's rotation axis. Over longer time periods of several months to years this axis can not be considered to be fixed in space as it performs a slow tumbling motion. Over the course of around 26000 years the axis will complete a full precession period so that it is facing in its initial direction again \cite{precession_definition} as visualized below:
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.40\linewidth]{images/precession.png}
		\caption{Tumbling movement of earth's rotation axis \cite{precession_definition}.}
		\label{fig:earth_axis_precession}
	\end{figure}
	\noindent
	Ignoring other (outside the scope of this paper) long term gravitational effects of the sun, moon and earth's equatorial bulge, the orbits of near earth satellite's trajectories should remain fixed to the background stars, more specifically the plane normal vector $n$. These properties will cause the vector $n$ and background stars to perform an apparent precession movement in the ECI coordinate system which has to be accounted for when performing star based camera calibration or comparing measured orbital properties to ground truth values defined further in the past.\newline
	Lieske provides a set of equations to formulate a rotation matrix with which ECI vectors can be multiplied with. It starts with the definition of the initial reference time $T$ and current time $t$ denoted in Julian centuries as follows:
	
	\begin{equation}
		\label{eq:precession_time_params}
		\begin{aligned}
			T = \frac{JD(\varepsilon_F) - JD(J2000)}{36525}\\
			t = \frac{JD(\varepsilon_D) - JD(\varepsilon_F)}{36525} 
		\end{aligned}
	\end{equation}
	Hereby, $JD(\varepsilon_F)$ marks the number of Julian days since J2000 for an initial starting epoch $\varepsilon_F$, whereas $\varepsilon_D$ marks the current time epoch. Our implementation of the star calibration procedure uses the BSC5P bright star catalog which defines star view vectors at $\varepsilon_F = J2000$ \cite{star_catalog} resulting in a collapse of the initial reference time to $T=0$.\newline
	With the time parameters established, three rotation angles $\zeta$, $z$ and $\sigma$ are defined by the following polynomials. Resulting angle values are denoted in arcseconds.
	
	\begin{equation}
		\begin{aligned}
			\zeta &= \left(2306.2181 + 1.39656\,T - 0.000139\,T^2\right) t
			+ \left(0.30188 - 0.000344\,T\right) t^2
			+ 0.017998\, t^3 \\[4pt]
			z &= \left(2306.2181 + 1.39656\,T - 0.000139\,T^2\right) t
			+ \left(1.09468 + 0.000066\,T\right) t^2
			+ 0.018203\, t^3 \\[4pt]
			\sigma &= \left(2004.3109 - 0.85330\,T - 0.000217\,T^2\right) t
			- \left(0.42665 + 0.000217\,T\right) t^2
			- 0.041833\, t^3
		\end{aligned}
	\end{equation}
	\noindent
	Next, let $R(\alpha)$ be the rotation matrix to rotate around the Z-axis by angle $\alpha$ and $Q(\alpha)$ the rotation matrix to rotate around the Y-axis by angle $\alpha$, the precession adjustment for any unprecessed ECI vector $v_0$ to obtain precessed $v$ is defined as follows:
	
	\begin{equation}
		v = R(-z)\cdot Q(\sigma)\cdot R(-\zeta)\cdot v_0
	\end{equation}
	\cite{precession_correction}
	\newline
	This precession correction must be applied to plane normal vectors $n$ of ground truth orbits defined further in the past to make them comparable to measurements. Furthermore, all view vectors of stars listed in a star catalog used for calibration must be precessed similarly.dx  
	...
	\end{equation}
	which satisfies the requirement of $-\pi \leq \theta_i \leq \pi$. Henceforth, this adaption makes the parametric model usable in a three dimensional environment, as angles and magnitudes can now be determined from vectors for the two dimensional model.\newline
	...
	\end{figure}
	\noindent
	Hereby, two cameras in different locations take an image of the satellite simultaneously. Immediately known are their position- and view vectors. These properties form two lines $l_1$ and $l_2$ defined as
	...
	\begin{enumerate}
		...
		\end{equation}
		\noindent
		Of this polynomial, a real number root $w$ larger than zero as solution for $x$  must be determined. If there is more than one solution $w$ it means that there is more than one physically feasible trajectory. The entire algorithm must be started over from step 1 exchanging one of the measurements for an additional one until a single solution $w$ has been determined. For computation, Curtis proposes using Newton's method and plotting the polynomial as function to determine a proper initial estimate of $x$. We propose using a solver like NumPy roots which determines the roots of any polynomial \cite{numpy_roots} and works out of the box.
	...
	\newline
	From here on onward the Gauss algorithm as presented by Curtis contains three additional steps to establish orbital properties from $r_1$, $r_2$ and $r_3$ determined in step 10. However, we have already established a method to determine orbital properties which we reuse to formulate a single final step as follows:
	...
	We start off with the circularity condition, meaning that all radii vectors $r_i$ must possess the same magnitude which formulates as follows: 
	\begin{equation}
	...
	\begin{thebibliography}{9}
		...
		\bibitem{precession_definition}
		University of Rochester. Precession of the Earth's Rotation Axis, 
		\newline{https://www.pas.rochester.edu/~blackman/ast104/precession.html}\newline
		Accessed 18.09.2026
		
		\bibitem{precession_correction}
		J.H. Lieske, Precession Matrix Based on IAU (1976) System of Astronomical Constants, 1978, \url{https://ui.adsabs.harvard.edu/scan/manifest/1979A&A....73..282L}\newline Accessed 18.09.2026
